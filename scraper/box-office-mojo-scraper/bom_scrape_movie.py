...
print("Parsing Box Office Mojo for movie metadata",
...
movie_nm, movie_id, str_mov_at, mov_tot = [], [
], mov_str_ind+0, mov_end_ind - mov_str_ind
...
all_links_movies = ['http://www.boxofficemojo.com/movies/?id=' +
                    m_id for m_id in movie_id]  # Convert id to link
...
while str_mov_at < mov_end_ind:
    ...
    # Taking care of case when increment greater than total movie end index
    if (counter + mov_inc_ind) > mov_end_ind:
    ...
    for idx, soup in enumerate(soups_movies):
        print('\t\t\tParsing movie:',
              movie_nm[str_mov_at + idx], '-', links_movies[idx])
        ...
        player_tags = list(soup.find_all(
            'a', href=re.compile(r"/people/\?view")))
        if player_tags:
            player_titles = [
                str(re.search(r"view=(.)*\&", str(x)).group())[5:-1] for x in player_tags]
            pl_vals = re.compile(
                "</table></div></div></td>(.*)").findall(str_soup)
            ...
            players.append({'Players': 'No Player Information'})
        ...
        if re.compile("Total Lifetime Grosses").findall(str_soup):
            ...
            tlg.append(
                {'Total Lifetime Gross': 'No Total Lifetime Gross Information'})
        ...
        if re.compile("Domestic Summary").findall(str_soup):
            ...
            ds.append({'Domestic Summary': 'No Domestic Summary Information'})
        ...
        if len(mpb_tags) > mpb_tags_pad and len(str(mpb_tags[mpb_tags_pad])) == 51:
            # In some pages, presence of extra empty tag (of length 51) is detected so we skip it
            mpb_tags_pad += 1
        if re.compile("Academy Awards®").findall(str_soup):
            awards.append(
                re.sub('</b>|<b>', '', str(mpb_tags[mpb_tags_pad].find_all('b')[0])))
            mpb_tags_pad += 1
        else:
            awards.append('No Academy Awards Information')
        ...
        if len(mpb_tags) > mpb_tags_pad and re.compile("Genre").findall(str(mpb_tags[mpb_tags_pad])):
            ...
            genre.append({'Genre': 'No Genre Information'})
        ...
        if len(mpb_tags) > mpb_tags_pad and re.compile("Franchise").findall(str(mpb_tags[mpb_tags_pad])):
            ...
            fran.append({'Franchise': 'No Franchise Information'})
        ...
        if len(mpb_tags) > mpb_tags_pad and re.compile("Chart").findall(str(mpb_tags[mpb_tags_pad])):
            ...
            chart.append({'Chart': 'No Chart Information'})
    ...
    print('\t\tWriting output of movies from', counter,
          'to', end_ctr_at, 'out of', mov_end_ind-1, '\n')
...
print("\nFinished scraping Box Office Mojo at",
      time.strftime("%Y/%m/%d %H:%M:%S"))
